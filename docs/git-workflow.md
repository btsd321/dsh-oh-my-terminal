...
本文档是 `dsh-oh-my-terminal` 仓库的 Git 工作流约定。AI agent 与人类开发者在本仓库中创建分支、提交代码、合并变更时**必须**遵循本规范。
...
| 前缀 | 用途 | 示例 |
|------|------|------|
| `feat/` | 新功能 | `feat/multi-tab-switching` |
| `fix/` | Bug 修复 | `fix/websocket-leak` |
| `refactor/` | 重构（不改行为） | `refactor/extract-shortcut-module` |
| `docs/` | 纯文档更新 | `docs/update-readme` |
| `test/` | 纯测试补充 | `test/shortcut-unit` |
...
5. **特性分支命名加编号**：便于追踪，如 `refactor/1-extract-shortcut`
...
1. 确认 develop 上所有改动已通过 typecheck、单元测试
...
```
refactor(shortcut): 提取 shortcut.ts 独立模块，消除 index.ts 重复解析代码

快捷键解析逻辑从 index.ts 提取到独立 shortcut.ts，便于单元测试与复用。
...
```
feat!(index): 支持多 shell 实例并发

BREAKING CHANGE: TerminalSession 构造函数新增 shellIndex 参数，
单实例初始化方式不再可用。
```
...
```
test(shortcut): 为 shortcut.ts 添加单元测试

覆盖基本功能、修饰键组合、特殊字符、边界情况。
```
...
```
merge(shortcut): develop → master — 快捷键解析模块提取
```
...
- [ ] 提交前运行 `pnpm run build` 更新 lib/ 构建产物，并将 lib/ 变更一并提交
