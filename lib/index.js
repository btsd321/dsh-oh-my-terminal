...
import { dirname, join as pathJoin2 } from "node:path";
...
function detectGitBash() {
  try {
    ...
    const { join, dirname: dirname2 } = __require("node:path");
    const { existsSync } = __require("node:fs");
    const gitDir = dirname2(dirname2(gitPath));
...
function apply(ctx) {
  ...
  const DATA_DIR = process.env[ENV_DATA_DIR] ?? pathJoin2(process.env.DSH_HOME ?? pathJoin2(homedir(), ".dsh"), "plugin-data", "terminal");
  ...
  async function createSession(options) {
    ...
    store.persistMeta();
    log2.info(`\u521B\u5EFA\u4F1A\u8BDD ${id}\uFF08${file}\uFF0C${cols}x${rows}\uFF0Ccwd=${sessionCwd}\uFF09`);
    pty.onData((data) => {
    ...
    pty.onExit(({ exitCode }) => {
      ...
      log2.info(`\u4F1A\u8BDD ${id} \u5DF2\u9000\u51FA\uFF08code ${exitCode}\uFF09`);
      for (const ws of record.wsClients) {
        try {
          ws.close(1e3, "session exited");
        } catch {
  ...
  async function restartSession(id, cwd) {
    ...
    log2.info(`\u91CD\u542F\u4F1A\u8BDD ${id}\uFF08\u7EE7\u627F ${seed.length} \u5B57\u7B26\u7F13\u51B2\uFF09`);
  ...
  const disposeRoute = webServer.register({
    ...
    async handler(req, res) {
      ...
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        const code = error instanceof TerminalError ? error.code : void 0;
  ...
  log2.info(`\u5BBF\u4E3B\u534A\u5DF2\u6FC0\u6D3B\uFF1B\u8DEF\u7531\u524D\u7F00 ${ROUTE_PREFIX}\uFF0CWS \u524D\u7F00 ${WS_PREFIX}\uFF0C\u5FEB\u6377\u952E ${runtimeSettings.toggleShortcut}\uFF0Cshell ${runtimeSettings.shellCommand || "(\u81EA\u52A8)"}`);
}
export {
  apply,
  inject,
