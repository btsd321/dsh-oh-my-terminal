 ...
 *
 *              设计目标：
 *              - 替代散落的 console.log/info/warn/error，统一为 debug/info/warn/error
 *                四种标准级别
 ...
 * log.info('创建会话', { id: 't1', cols: 80, rows: 24 });
 * // → [2025-01-15T07:02:03.456Z] [INFO] [terminal-host] 创建会话 {"id":"t1","cols":80,"rows":24}
 * ```
 *
 * 分层约束：本文件属基础层，不依赖任何上层模块。
 *
 * 安全纪律：**终端输入/输出数据绝不进日志**——pty 输出与 WebSocket 数据帧
 * 是用户会话内容（可能含密码），只记生命周期事件（创建/退出/重启/删除）与 id。
 */
 ...
 *
 * Node 环境下 console.info/warn/error 默认输出到 stderr；浏览器环境下
 * 输出到 DevTools Console——两端 API 一致，无需环境判断。
 *
 * @param module - 模块标签（如 'terminal-host'、'terminal-client'）
 * @returns 日志器实例
 */
